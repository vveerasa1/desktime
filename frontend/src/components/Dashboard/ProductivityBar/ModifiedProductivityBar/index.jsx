...
import { Customized, XAxis,ReferenceArea } from "recharts";
...
const ProductivityBar = ({ data = productivityData, getProductiviyData }) => {
  const normalizedData = timeSlots.map((slot) => {
    const productivity = getProductiviyData?.data.session;
    const existing = Array.isArray(productivity)
      ? productivity.find((item) => item.time === slot)
      : null;
  ...
  const rawProductivity = getProductiviyData?.data.session || productivityData;

const activeRanges = rawProductivity
  .filter((d) => d.timeRange)
  .map((d) => {
    const [start, end] = d.timeRange.split(" - ");
    return {
      start,
      end,
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            /> */}
              <XAxis
                ...
                axisLine={false}
                tickLine={false}
                ticks={timeLabels} // optional: show only certain time labels
                tick={{ fontSize: 10, fill: "#666" }}
              />
              {console.log(activeRanges,"TIME RANGE")}
              {activeRanges.map((range, idx) => (
                <ReferenceArea
                  key={idx}
                  x1={range.start}
                  x2={range.end}
                  y1={200} // adjust y1/y2 to draw below bars
                  y2={250}
                  fill="#4caf50"
                  fillOpacity={0.6}
                  ...
                  return (
                    <g>
                      {normalizedData.map((d, i) => {
                        const hasData =
                          d.productive > 0 || d.neutral > 0 || d.break > 0;
