...
const timeToMinutes = (timeString) => {
  ...
  if (typeof timeString === "string" && timeString.includes("m") && timeString.includes("s")) {
...
function renderDayCell(arg, apiSessionData, userData) {
  console.log(arg,"ARG")
  console.log(apiSessionData,"API SESSION DATA")
...
const dateStr = arg.date.toLocaleDateString('en-CA');
  ...
  const minimumHoursMinutes = userData?.minimumHours ? parseInt(userData.minimumHours.split(' ')[0]) * 60 : 0;
  const trackingStartTimeMinutes = userData?.trackingStartTime ? timeToMinutes(userData.trackingStartTime) : 0;
  ...
  if (data?.status === 'working') {
  ...
  } else if (!data || (data.status === 'completed' && (!data.arrival && !data.worked))) {
    ...
    if (arrivalMinutes > 0 && arrivalMinutes < trackingStartTimeMinutes) { // Only apply if there's an actual arrival time
        ...
        {statusClass === styles.absent ? (
          ...
          <Box className={styles.textAlign}>
            <Box className={styles.progressBarContainer}>
              ...
              {(() => {
                const effectiveDeskTimeMinutes = deskTimeMinutes > 0 ? deskTimeMinutes : 1; // Avoid division by zero
                const workedPercentage = (workedMinutes / effectiveDeskTimeMinutes) * 100;
                const idlePercentage = 100 - workedPercentage;
                return (
                  <>
                    <Box className={styles.greenBar} style={{ width: `${workedPercentage}%` }}></Box>
                    <Box className={styles.grayBar} style={{ width: `${idlePercentage}%` }}></Box>
                  </>
                );
              })()}
            </Box>
...
export default function EmployeeCalendar({ getProductiviyData }) {
  ...
  const customDayCellContent = (arg) => renderDayCell(arg, apiSessionData, userData);
