...
ipcMain.on('token', (event, token) => {
  ...
  if (electronAuthToken && !global.isTrackingStarted) {
    ...
    event.sender.send('token-response', 'Token received (tracking already started or token not present)');
  }
});

function createWindow() {
  const preloadPath = path.join(__dirname, 'preload.js');
  console.log('Main: Attempting to create BrowserWindow with preload path:', preloadPath); // Added detailed preload path log
  ...
  try {
    ...
    console.error('Main: FATAL ERROR during BrowserWindow creation or setup:', error);
    app.quit();
  }
}
...
async function checkIfSessionEnded() {
  ...
  if (!sessionId || sessionEnded || !token) {
    if (!token) console.warn('checkIfSessionEnded skipped: No token available.');
    return;
  }
  try {
    ...
    console.error('[Session Check Error]', err.response ? err.response.data : err.message);
  }
}
...
async function sendActivityToServer(data) {
   const token = getAuthTokenForMainProcess();
   if (!sessionId || sessionEnded || !token) {
     if (!token) console.warn('Activity upload skipped: No token available in main process.');
     return;
   }
   ...
   if (data.type === 'idle') {
    ...
    if ((now - idleCheckStart) >= IDLE_THRESHOLD && !idleStart) {
      ...
      if (activeStartTime) {
        ...
        }).catch(err => console.error('[Active Update Error]', err.response ? err.response.data : err.message));
    ...
    if (idleStart) {
      ...
      }).catch(err => console.error('[Idle Update Error]', err.response ? err.response.data : err.message));
    }
    idleStart = null;
    idleCheckStart = null;
    if (!activeStartTime) {
...
async function startTracking() {
  ...
  setInterval(async () => {
    try {
      ...
      if (!token) {
        console.warn('Screenshot upload skipped: No token available in main process.');
        return;
      }
      ...
      console.error('[Screenshot Error]', err.response ? err.response.data : err.message);
    }
  }, 5 * 60 * 1000);
}
...
async function initializeDailyTracking(userId) {
  ...
  if (!token) {
    console.error('Initialization skipped: No token available to initialize tracking session.');
    return;
  }
  try {
    ...
    console.error('[Tracking Init Error]', err.response ? err.response.data : err.message);
  }
}
